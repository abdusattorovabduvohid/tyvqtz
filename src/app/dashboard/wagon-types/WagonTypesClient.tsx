...
import type { WagonTypeRow as WagonType } from "@/lib/wagon-types-list";
...
// Справочник приходит вместе со страницей — её собирает сервер.
export function WagonTypesClient({
  initialTypes,
}: {
  initialTypes: WagonType[];
}) {
  const can = useCan();
  const { t, lang } = useI18n();
  const [types, setTypes] = useState<WagonType[]>(initialTypes);
  ...
  // Перечитываем только после своих действий.
  async function load() {
    try {
