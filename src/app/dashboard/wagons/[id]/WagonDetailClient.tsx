...
// Карточка приходит вместе со страницей — её собирает сервер. Дальше
// экран освежается опросом, как и раньше.
export function WagonDetailClient({
  initial,
}: {
  initial: WagonDetailResponse;
}) {
  ...
  const [wagon, setWagon] = useState<WagonDetail | null>(initial.wagon);
  ...
  const load = useCallback(async () => {
    try {
      const r = await apiFetch<WagonDetailResponse>(`/api/wagons/${id}`);
    ...
    <Page>
      ...
      <Card mb="lg" p="lg">
        <Group justify="space-between" wrap="wrap">
          ...
          <Group>
            <ActionIcon variant="light" size="lg" onClick={() => load()} title={t("common.refresh")}>
              <IconRefresh size={18} />
            </ActionIcon>
        ...
        >
          {wagon.stages.map((s, stageIdx) => {
            ...
            return (
              ...
              >
                <Card withBorder p="md" radius="md" className="reveal-right">
                  {/* Шапка позиции */}
