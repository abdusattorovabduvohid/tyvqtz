...
// Список уже пришёл вместе со страницей (её собрал сервер), поэтому здесь
// нет загрузки при открытии — только перечитывание после своих действий.
export function WagonsClient({
  initialWagons,
  types: typeOptions,
}: {
  initialWagons: Wagon[];
  ...
  // Типы вагонов меняются раз в год, поэтому после создания и удаления
  // перечитываем только сами вагоны.
  const load = useCallback(async () => {
    try {
      const w = await apiFetch<{ wagons: Wagon[] }>("/api/wagons");
      setWagons(w.wagons);
    } catch (e) {
      showError(e);
    }
