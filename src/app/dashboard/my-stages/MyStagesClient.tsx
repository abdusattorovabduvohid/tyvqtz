...
// Данные приходят вместе со страницей — её собирает сервер. Дальше
// список освежается опросом раз в двадцать секунд, как и раньше.
export function MyStagesClient({ initial }: { initial: MyStagesResponse }) {
  const { t, lang } = useI18n();
  const [tasks, setTasks] = useState<MyTask[]>(initial.tasks);
  const [mine, setMine] = useState<MyStage[]>(initial.mine);
  const [creations, setCreations] = useState<MyCreation[]>(initial.creations);
