...
import {
  ...
  type MyCreation,
  type MyStage,
  type MyStagesResponse,
  type MyTask,
} from "@/lib/api-types";
import { wagonSchedule, splitWorksIntoDays } from "@/lib/format";

// Лежит отдельно от роута: этими же данными страница собирается на
// сервере, чтобы телефон не ходил за ними вторым заходом.
//
...
export async function getMyStages(user: {
  id: string;
}): Promise<MyStagesResponse> {
  ...
  return { tasks, mine, creations, serverNow: now };
}
